 ...
 */
void fillvectors(double const& expected, double const& actual, double const& sigma, vector<double >& n_distribution, vector<double >& n_sigma_distribution, vector<double >& expected_distribution){

	double dif = (expected-actual);
	double dif_sig = dif/sigma;
	n_distribution.push_back(dif_sig);
...
void fitBeta(TH1D &h, vector<double> const& x, double &mean, double &sigma){
	...
	int tallestbin = 0;
	maxBinProperties(h, tallestbin, tallestbinheight, tallestbincenter);
...
void generateDistributions(TTree &t1, TTree &t2, ParticleEvent &originals, TrackRecons& reconstructions, std::vector<std::vector<double> * > &distributions, int start, int end){
	...
	auto expected_theta = 0., expected_area = 0., actual_theta = 0., actual_area = 0., sigma_theta = 0.01, sigma_area = 0.;
	...
	for (unsigned ev = start; ev < end; ++ev){
		...
		for (unsigned int p = 0; p < pars.size(); ++p){
			...
			fillvectors(expected_theta, actual_theta, sigma_theta, *distributions.at(0), *distributions.at(1), *distributions.at(2));

			fillvectors(expected_area, actual_area, sigma_area, *distributions.at(3), *distributions.at(4), *distributions.at(5));
		}
	}
}
...
void calibrateSigmas(TTree& t1, TTree& t2, ParticleEvent& originals, TrackRecons& reconstructions, string const &particletype, string const &prefix, int &startevent){
	...
	std::string theta_n_distribution_file = "theta_n_distribution.pdf";
	std::string theta_n_sigma_distribution_file = "theta_n_sigma_distribution.pdf";
	std::string theta_expected_file = "theta_expected.pdf";
	std::string area_n_distribution_file = "area_n_distribution.pdf";
	std::string area_n_sigma_distribution_file = "area_n_sigma_distribution.pdf";
	...
	for (map<int, string>::iterator i = histtype.begin(); i != histtype.end(); ++i){
		...
		hists.push_back(makehist(*distributions.at(indx), filenames.at(indx), 500));
		TH1D &hist = hists.back();
		// normalizehist(hist, 100);
		if (type == "emission_angle"){
			fitBeta(hist, *distributions.at(indx), theta_fit_difference, theta_fit_sigma);
		}
		...
		for (unsigned int p = 0; p < pars.size(); ++p){
			...
			for (unsigned int i = 0; i < recon.Options.size(); ++i){
				...
				if (name == particletype){
					...
					// (expected_photon_fit_difference - old_expectedphoton_difference)/expected_photon_fit_sigma;
					recon.delSigArea.at(i) = abs(recon.delSigArea.at(i));

					// expected emission angle
					double old_theta_difference = (expected_theta - actual_theta);

					recon.delSigTheta.at(i) = 1.2;// -= theta_fit_difference;
					// (theta_fit_difference - old_theta_difference)/theta_fit_sigma;
					recon.delSigTheta.at(i) = abs(recon.delSigTheta.at(i));
					recon.Sigmas.at(i) = sqrt(pow(recon.delSigTheta.at(i),2)+pow(recon.delSigArea.at(i),2));

					if ( true || ((recon.delSigTheta.at(i) < 1.5) && (recon.delSigArea.at(i) < 1.5))){
						++matches;
